/*
 		...
 		3) 메소드 제작시 고민
 			...
 			----------------------------------------
 					...
 					=> 리턴형으로 사용
 					   ...
 					   참조형 : int[],boolean[]
 					   ----------------------
 					   리턴형은 한개만 사용할 수 있다(정수를 여러개 리턴할 시 묶어서 리턴)
 			----------------------------------------
 					O				X
 					=> double Math.random()
 					=> String trim()
 		...
 		4) 메소드 선언 및 호출 방법
 			public class A{
				...
				--------------------------------
 					메소드 선언 위치 (매소드 안에 메소드 선언은 불가능)
 					클래스안에 선언
 				--------------------------------
 				클래스 종속 => 메소드(Java) => 독립적 사용이 불가능
 				클래스 밖에서 선언 => 함수(C/C++)
 			}
