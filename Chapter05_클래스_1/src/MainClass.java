/*
 		...
 		2. 데이터 관리를 위한 사용자 정의 데이터형
 		   ...
 		   = 클래스의 변수의 종류
 		     ...
 		     --------------------------------------------------------
 		     인스턴스변수 => new : 새로운 공간이 생성 => 클래스 전체 사용 가능
 		     => 클래스를 메모리에 저장한 상태(인스턴스)
 		     		class ClassName{
 		     			int a;
 		     		}
 		     정적변수(static,공유변수) => 컴파일시 자동 저장
 		     		class ClassName{
 		     			static int a;
 		     		}
 		     지역변수 : 메소드 호출시마다 생성 => 메소드 안에서만 사용이 가능
 		     		class ClassName{
 		     			void display(){
 		     				int a;
 		     			}
...
/*
 		파일명은 .java
 			  ------
 			  => 저장명은 클래스명과 동일(대소문자구분)
 			  A.java a.java
 			  
 		한 파일 안에 여러개의 클래스를 사용할 수 있다
 		class A
 		public class B
 		-------------- public class는 한 파일안에 1개만 사용할 수 있다
 					   ------------ 파일명이 된다
 		
 		class A
 		class B
 		------- 저장명은 A.java, B.java
 		------- 일반적으로는 main이 있는 클래스명이 파일명이 된다
 		...
 		*** 같은 폴더(src)
 		...
 		----------------------- 오류(A는 이미 사용중)
 		같은 폴더안에 동일한 이름의 class는 한개만 사용가능
