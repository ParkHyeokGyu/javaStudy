 		...
 		자바 => 클래스 기반
 			  -----
 			  	...
 			  	2. 메소드 : 특정 작업을 수행하는 일련의 명령문의 집합
 			  	     ...
 			  	     | 메소드 구조
 			  	     	...
 			  	     	= 매개변수 => 기본형,참조형(배열,클래스)
 			  	     		...
 			  	     		사용자의 요청에 필요한 데이터
 			  	     	= 메소드명 => 식별자를 이용한다
 			  	     	------------------------- 사용자의 요청에 대한 처리(한가지 기능 수행)
 			  	     	= void method(){
 			  	     		return; => 생략 가능 => 메소드 처리 종료
 			  	     				   ------ 컴파일러에 의해 자동으로 추가
 			  	     	  }
 			  	     	  int method(){
 			  	     		return 10;
 			    ...
 			    3. 메소드 유형
 			    	...
 			    	void method(){
 			    		int a=10; => 지역변수(반드시 초기화를 해야 사용 가능)
 			    	} => a는 메모리에서 해제(메모리 자체 해제 => Stack)
 			    	
 			    	매개변수는 지역변수와 동일 취급
 			    	return은 메소드 종료를 알려준다
 			    	메소드 블록이 닫히기전에 return을 사용한다
 			    	=> 중간에 사용을 해서 미리 종료할수 있다
