...
public class 문자열_4 {
	public static void main(String[] args) {
		...
		System.out.println("==============================================");
		// 2. 노래명중에서 가장 긴 제목의 노래와 가수명,앨범명 출력
		max=0;
		for(String t:title) {
			if(max<t.length()) {
				max=t.length();
		...
		System.out.println("하강한 노래의 갯수:"+cnt);
		System.out.println("==============================================");
		// 5. 임영웅의 노래 제목을 출력
		cnt=0;
		for(int i=0;i<title.length;i++) {
		...
		System.out.println("임영웅의 곡은 총 "+cnt+"곡 입니다");
		System.out.println("==============================================");
		// 6. 검색어를 입력받아 => 검색어가 포함되어있는 노래명 출력
		cnt=0;
		System.out.print("검색어 입력:");
		String findData=sc.next();
		for(int i=0;i<title.length;i++) {
			if(title[i].contains(findData)) {
				System.out.println((i+1)+"."+title[i]+" "+singer[i]);
				cnt++;
			}
		}
		System.out.println(findData+"이(가) 포함된 곡의 총 갯수는 "+cnt+"곡 입니다");
	}
}
