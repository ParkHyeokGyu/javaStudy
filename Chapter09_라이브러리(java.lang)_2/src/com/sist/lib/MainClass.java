/*
 	...
 	=> System : 출력,메모리 해제,프로그램 종료
 	            =*println() : 에러 출력,멤버변수 값 확인,전송 값 확인
 	                          윈도우/브라우저 => 자체 출력 => 디버깅역할
 	            = gc() : 가비지 컬렉션을 호출하여 메모리를 비워준다
 	                     멀티미디어,애니메이션
 	                     => 실시간 도로 cctv
 	            = exit() : 종료
 	                  -- 0(정상),비정상
 	=> Math : 수학 => 모든 메소드가 static으로 되어있다
 	          메모리 할당을 하지 않고 클래스 이름으로 접근하여 메소드를 사용한다
 	          => Math.메소드명()
 	          =*random() : 리턴형(double => 0.0~0.00)
 	          =*ceil() : 올림 => 총페이지 구하기
...
 		*public boolean startsWith(String s)
	...
	3. 	두개의 String에 값만을 비교해서 같으면 true, 다르면 false를 반환한다.(대소비교)
		...
		=> 객체에는 Heap에 저장되어 있는 메모리 주소를 가지고 있다
		                         --------- 참조 변수
		(주소 비교시에는 ==,실제 저장된 값을 비교할때는 equals)
	...
	6. 	문자열의 길이를 반환한다.
		*public int length()
		=> 비밀번호 / 글쓰기...
		  -------  -----
		  몇 자 이상  오라클의 지정된 문자 갯수 초과
		            => 오라클은 변수 선언 시 변수의 크기를 지정
...
 
	...
	15. 지정한 개체의 원시 값을 반환 (데이터형을 문자열로 변경)
		toString() => Object가 가지고있는 toString() 오버라이딩
		---------- 문자열 변환 => 데이터형을 문자열로 변경
	 
