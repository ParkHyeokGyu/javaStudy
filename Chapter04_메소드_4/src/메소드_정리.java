/*
 		...
 		=> 매개변수 전송
 			int c=add( 10,   20) => 호출시에는 매개변수값을 전송
 					    |	  |
 			int add(int a,int b){ => 임시 메모리 공간 => 지역 변수의 일종
 				return a+b; => c에 저장
 			}
 		=> 입력값(O), 리턴형(O)
 		=> 입력값(X), 리턴형(O)
 		...
 		메소드 정의
 		1. 특정 작업을 수행하는 명령문들의 집합
 		2. 메소드를 호출할 경우에만 매개변수가 메모리에 할당된다
 		...
 		Call By Value
 		= 일반 데이터형 => 값 참조(자제는 변경이 안됨) => 다른 메모리에 생성
 		
 		---------------------------------------------------
 		절차적				구조적				객체 지향
 		한줄씩 나열				같은 기능별(메소드)		변수+메소드
