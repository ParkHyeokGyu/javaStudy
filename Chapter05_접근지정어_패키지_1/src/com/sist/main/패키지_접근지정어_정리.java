...
/*
		...
		protected	같은 패키지 안에서 접근이 가능
					상속을 받은 경우에는 상속 내린 클래스의 패키지까지 접근 가능
					사용빈도가 거의 없다
		public		패키지와 상관없이 모튼 클래스에서 접근이 가능
		...
		패키지 : 폴더의 개념 => 관련된 클래스를 모아 저장하는 공간
						  -----------
		...
		=> 패키지를 사용하면
		   ...
		   다른 패키지 => import를 사용하여 클래스를 읽어 온다
		...
		*** 패키지명은 com(org).회사명.압축
		...
		------------
		aa.a=100; // 메모리에 값 저장
		System.out.prinln(aa.a); // 메모리에서 값 읽기
		=> 저장 / 읽기 => 메소드
		   getter / setter
