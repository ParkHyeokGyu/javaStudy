...
public class MainClass_1 {
	public static void main(String[] args) {
		...
		// 요일, 입력달의 마지막 날짜 얻기 => Calendar에 날짜를 설정
		// cal.set(),cal.get()
		/*
		 	1.년도 설정 set(Calendar.YEAR,year)
		 	2.월 설정 set(Calendar.MONTH,month-1) => month는 0번부터 시작
		 	3.일 설정 set(Calendar.DATE,1) => 달력을 만들때 1일로 지정
		 	
		 	=> Calendar => 추상 클래스
		 	               -------- 미완성 클래스(메소드 구현이 안된 상태)
		 	               new를 이용하여 메모리 할당을 할 수 없다
		 	=> new를 사용하지 않고 메모리 할당(추상 클래스,인터페이스)
		 */
		// Calendar 객체 생성
